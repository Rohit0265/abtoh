...
async function getSongs(folder) {
    ...
    try {
        ...
        return data[folderName].map(file => `${folder}/${file}`);
    } catch (error) {
...
async function display1() {
    try {
        ...
        for (let folderName in data) {
            ...
            cardsop.innerHTML += `
                <div data-folder="${folderName}" class="cards">
                    <div class="play">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                             width="24" height="24" color="#000000" fill="none">
                            <path d="M18.8906 12.846C18.5371 14.189 16.8667 
                                     15.138 13.5257 17.0361C10.296 18.8709 
                                     8.6812 19.7884 7.37983 19.4196C6.8418 19.2671 
                                     6.35159 18.9776 5.95624 18.5787C5 17.6139 5 
                                     15.7426 5 12C5 8.2574 5 6.3861 5.95624 5.42132C6.35159 
                                     5.02245 6.8418 4.73288 7.37983 4.58042C8.6812 4.21165 
                                     10.296 5.12907 13.5257 6.96393C16.8667 8.86197 18.5371 
                                     9.811 18.8906 11.154C19.0365 11.7084 19.0365 12.2916 
                                     18.8906 12.846Z"
                                  stroke="#141B34" stroke-width="1.5" fill="#000000" 
        ...
        Array.from(document.getElementsByClassName("cards")).forEach(e => {
            e.addEventListener("click", async item => {
                ...
                if (songs.length > 0) {
                    ...
                    const firstSongName = songs[0].split(`${currFolder}/`)[1]
                        .replaceAll("%20", " ").replace(".mp3", "");
...
async function main() {
    ...
    songs = await getSongs("/Songs/MyList");
    if (songs.length > 0) {
        refreshPlaylistUI(songs);
